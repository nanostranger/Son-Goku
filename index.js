...
function calculateTypingDelay(responseText) {
    ...
    return Math.min(5000, 2000 + (length / 50)); // More realistic typing speed
}
...
client.on('messageCreate', async message => {
    ...
    try {
        ...
        // FIXED: Make message splitting much rarer (only 2% chance) and only for longer messages
        const splitChance = Math.random() < 0.02; // Reduced from 5% to 2%
        if (responseMessages.length === 1 && splitChance && responseText.length > 200) { // Increased minimum length
